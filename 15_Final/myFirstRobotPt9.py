...
window.title("Knight Security Agent // BBot // OPS Center")  # Set title of window as the name of the App
window.configure(bg="#0a0e1a")        # dark navy background
...
canvas = tk.Canvas(window, width=num_cols * cell_size, height=num_rows * cell_size,
                   bg="#0d1220", highlightthickness=1, highlightbackground="#1e3a5f")
...
def draw_grid():
    ...
    for i in range(num_rows + 1):
        canvas.create_line(0, i * cell_size, num_cols * cell_size, i * cell_size, fill="#1e3a5f")
    # Draw vertical gridlines
    for i in range(num_cols + 1):
        canvas.create_line(i * cell_size, 0, i * cell_size, num_rows * cell_size, fill="#1e3a5f")
    for row in range(num_rows):
        for col in range(num_cols):
            x = col * cell_size + cell_size / 2
            y = row * cell_size + cell_size / 2
            canvas.create_text(x, y + 30, text="Row" + str(row) + ",Col" + str(col), fill="#1e3a5f")
...
def draw_robot():
    ...
    for row in range(num_rows):
        for col in range(num_cols):
            ...
            g_val = max(0, 18 - heat // 14)    # stays near 0 for dark background
            b_val = max(0, 32 - heat // 8)
            color = f"#{min(heat + 10, 58):02x}{g_val:02x}{b_val:02x}"
    ...
    for row in range(num_rows):
        for col in range(num_cols):
            ...
            canvas.create_text(rx, ry, anchor="nw", text=str(rewards[row][col]), fill="#ff4444", font=("Courier", 9, "bold"))
    ...
    canvas.create_rectangle(x1, y1, x2, y2, fill="#003d66", outline="#00d4ff", width=2)
    canvas.create_text(x1 + cell_size//2, y1 + cell_size//2, text="BBot", fill="#00d4ff",
...
info_label = tk.Label(window, text="[ AWAITING DEPLOYMENT ORDER ]",
                      font=("Courier", 11, "bold"), fg="#7ab8d4", bg="#060b14",
                      pady=6, relief="flat", bd=0)
info_label.pack(fill="x") # Pack label on canvas
 
# ***** Create label to tell the user they can enter the number of moves *****
moves_label = tk.Label(window, text="// MISSION LENGTH (moves)", font=("Courier", 9),
                       fg="#4a90a4", bg="#0a0e1a", pady=4)
moves_entry = tk.Entry(window, font=("Courier", 13), width=6, justify="center",
                       bg="#0d1220", fg="#00d4ff", insertbackground="#00d4ff",
                       relief="flat", bd=1, highlightthickness=1,
                       highlightbackground="#1e3a5f", highlightcolor="#00d4ff")
...
button_frame = tk.Frame(window, pady=8, bg="#060b14")
button_frame.pack()

# Let the user know what the speed options are
speed_label = tk.Label(window, text="// SWEEP VELOCITY", font=("Courier", 9),
                       fg="#4a90a4", bg="#0a0e1a", pady=4)
...
speed_menu.config(font=("Courier", 10), width=25,
                  bg="#0d1220", fg="#7ab8d4",
                  activebackground="#1e3a5f", activeforeground="#00d4ff",
                  relief="flat", bd=1, highlightthickness=1,
                  highlightbackground="#1e3a5f")
# Style the dropdown menu itself
speed_menu["menu"].config(bg="#0d1220", fg="#7ab8d4",
                          activebackground="#1e3a5f", activeforeground="#00d4ff",
                          font=("Courier", 10))
...
def run_episode():
    ...
    if current_move >= TOTAL_MOVES:
        ...
        info_label.config(text=f"MOVE {current_move} OF {TOTAL_MOVES}  |  BBOT: ROW {robot_row}, COL {robot_col}", fg="#7ab8d4")        # Re-enable the reset button now that the episode is finished
    ...
    info_label.config(text=f"[ MISSION COMPLETE — {TOTAL_MOVES} MOVES LOGGED — RESET TO REDEPLOY ]", fg="#00ff88")
...
def start_simulation():
    ...
    info_label.config(text="[ BBOT DEPLOYING... SWEEP IN PROGRESS ]", fg="#a070ff")
...
# Start button
start_btn = tk.Button(button_frame, text="▶  DEPLOY", font=("Courier", 12, "bold"),
                      bg="#003d1f", fg="#00ff88", activebackground="#004d25",
                      activeforeground="#00ff88", padx=20, pady=6,
                      relief="flat", bd=0, cursor="hand2", command=start_simulation)
# Now place the button
start_btn.pack(side="left", padx=10)

# Reset button
reset_btn = tk.Button(button_frame, text="↺  RESET", font=("Courier", 12, "bold"),
                      bg="#1a1a1a", fg="#555555", activebackground="#222222",
                      activeforeground="#777777", padx=20, pady=6,
                      relief="flat", bd=0, cursor="hand2", command=reset_simulation)# Now place the button
